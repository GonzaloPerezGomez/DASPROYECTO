...
import com.example.dasproyecto.services.workers.ConexionWorker;
...
public class DBmanager {
    ...
    /**
     * Sincroniza el token FCM del dispositivo con el servidor remoto.
     * Se ejecuta en segundo plano mediante WorkManager.
     */
    public void actualizarFCMTokenRemoto(int usuarioId, String token) {
        if (usuarioId == -1 || token == null || token.isEmpty()) return;

        Data datos = new Data.Builder()
                .putString("accion", "actualizar_fcm_token")
                .putInt("usuario_id", usuarioId)
        ...
        WorkManager.getInstance(context).enqueue(req);
        Log.d(TAG, "Tarea de actualización de token FCM encolada para usuario: " + usuarioId);
    }
}
